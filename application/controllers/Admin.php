...
class Admin extends CI_Controller
{
    ...
    public function pendaftar($id)
    {
        if ($this->session->userdata('nama')) {
            if ($this->session->userdata('admin')) {
                $data['pendaftar'] = $this->db->select('tbl_lamaran.*,tbl_akun.* EXCEPT status')->join('tbl_akun', 'tbl_akun.id_akun=tbl_lamaran.id_akun', 'left')->where('tbl_lamaran.id_loker', $id)->get('tbl_lamaran')->result();
